...
<template>
    <AppLayout title="Cheques">
        <section>
            <div class="w-full lg:flex lg:items-end lg:justify-between">
                <div>
                    <div class="flex items-center gap-x-3">
                        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Cheques</h2>

                        <span class="px-3 py-1 text-xs font-semibold rounded-full text-emerald-700 bg-emerald-100/60 dark:bg-gray-800 dark:text-emerald-400">{{ cheques_count }} Cheques</span>
                    </div>
                ...
                <div class="mt-4 sm:flex sm:items-center sm:justify-between sm:gap-x-4 lg:mt-0">
                    <div class="flex overflow-hidden bg-white border divide-x rounded-lg md:w-auto sm:w-1/2 dark:bg-gray-900 rtl:flex-row-reverse dark:border-gray-700 dark:divide-gray-700">
                        <button @click="filters.type = ''" :class="filters.type === '' || filters.type === null ? 'bg-gray-100' : 'hover:bg-gray-100'" class="px-5 w-1/3 md:w-auto shrink-0 py-2.5 text-xs font-semibold text-gray-600 transition-colors duration-200 sm:text-sm">
                            All
                        </button>

                        <button @click="filters.type = 1" :class="filters.type == 1 ? 'bg-gray-100' : 'hover:bg-gray-100'" class="px-5 w-1/3 md:w-auto shrink-0 py-2.5 text-xs font-semibold text-gray-600 transition-colors duration-200  sm:text-sm">
                            Credit
                        </button>

                        <button @click="filters.type = 0" :class="filters.type === 0 ? 'bg-gray-100' : 'hover:bg-gray-100'" class="px-5 w-1/3 md:w-auto shrink-0 py-2.5 text-xs font-semibold text-gray-600 transition-colors duration-200 sm:text-sm">
                            Debit
                        </button>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 gap-6 mt-8 md:grid-cols-2 2xl:grid-cols-3">
                <Cheque
