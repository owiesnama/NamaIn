...
const submit = () => {
    ...
    form.post(route("sales.return.store", props.invoice.id));
};
</script>

<template>
    <AppLayout :title="__('Return Sale')">
        <!-- Page Header -->
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __("Return Sale Invoice") }} #{{ invoice.serial_number }}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __("Create a return for customer") }}: <span class="font-medium text-gray-700 dark:text-gray-300">{{ invoice.invocable.name }}</span></p>
            </div>
        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-none p-5">
                    ...
                    <div>
                        ...
                        <TextArea
                            ...
                            class="block w-full"
                            rows="4"
                            :placeholder="__('Explain why items are being returned...')"
                            required
                        />
