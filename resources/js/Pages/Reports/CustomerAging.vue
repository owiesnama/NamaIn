...
const props = defineProps(['data', 'summary', 'filters', 'customers']);
...
watch(localFilters, (val) => {
    ...
    debounceTimer = setTimeout(() => {
        ...
        router.get(route('reports.customer-aging'), params, { preserveState: true, preserveScroll: true });
    }, 400);
}, { deep: true });
...
function requestExport() {
    router.post(route('exports.store'), {
        export_key: 'report-customer-aging',
        format: 'xlsx',
...
<template>
    <AppLayout :title="__('Customer Aging')">
        <div class="space-y-6">
            <!-- Header -->
            <div class="w-full lg:flex lg:items-center lg:justify-between">
                <div>
                    <div class="flex items-center gap-x-3">
                        ...
                        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">{{ __('Customer Aging') }}</h2>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div class="px-4 py-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 sm:p-6">
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('Customers') }}</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ summary?.customer_count ?? 0 }}</p>
                </div>
                <div class="px-4 py-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 sm:p-6">
