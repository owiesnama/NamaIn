...
const props = defineProps(['data', 'summary', 'filters', 'storages', 'categories']);
...
watch(localFilters, (val) => {
    ...
    debounceTimer = setTimeout(() => {
        ...
        router.get(route('reports.inventory-valuation'), params, { preserveState: true, preserveScroll: true });
    }, 400);
}, { deep: true });
...
function requestExport() {
    router.post(route('exports.store'), {
        export_key: 'report-inventory-valuation',
        format: 'xlsx',
...
<template>
    <AppLayout :title="__('Inventory Valuation')">
        <div class="space-y-6">
            <!-- Header -->
            <div class="w-full lg:flex lg:items-center lg:justify-between">
                <div>
                    <div class="flex items-center gap-x-3">
                        ...
                        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">{{ __('Inventory Valuation') }}</h2>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="px-4 py-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 sm:p-6">
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('Total Value') }}</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ formatCurrency(summary?.total_value) }}</p>
                </div>
                <div class="px-4 py-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 sm:p-6">
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('Products') }}</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ summary?.product_count ?? 0 }}</p>
                </div>
                <div class="px-4 py-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 sm:p-6">
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('Total Items') }}</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ summary?.total_items ?? 0 }}</p>
                </div>
            </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50/50 dark:bg-gray-800/40">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-start text-[10px] font-bold uppercase tracking-[0.1em] text-gray-400 dark:text-gray-500">{{ __('Avg Cost') }}</th>
                                        <th class="px-6 py-4 text-start text-[10px] font-bold uppercase tracking-[0.1em] text-gray-400 dark:text-gray-500">{{ __('Total Value') }}</th>
                                    </tr>
                                </thead>
