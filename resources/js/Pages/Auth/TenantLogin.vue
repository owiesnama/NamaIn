...
<template>
    <Head :title="__('Log In') + ' — ' + tenant.name" />
    ...
    <AuthenticationCard :dir="direction">
        ...
        <template #nav-actions>
            <span class="text-sm font-medium text-slate-500">{{ tenant.name }}</span>
        </template>

        <!-- Icon + Tenant Branding -->
        <div class="mb-6 flex flex-col items-center text-center">
            ...
            <h1 class="mt-4 text-2xl font-bold text-slate-900">{{ tenant.name }}</h1>
            <p class="mt-1.5 text-sm text-slate-500">{{ __("Sign in to your account") }}</p>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="flex items-center">
                <input
                    ...
                    type="checkbox"
                    class="border-slate-300 rounded text-emerald-600 focus:border-emerald-300 focus:ring focus:ring-emerald-200 focus:ring-opacity-50"
                />
