    ...
    <Modal :show="show" @close="emit('new-sale')">
        <div class="p-6 text-center">
            ...
            <div class="flex gap-x-3">
                <button @click="emit('print')" class="flex-1 inline-flex items-center justify-center gap-x-2 py-2.5 text-sm font-semibold text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                ...
                <button @click="emit('new-sale')" class="flex-1 py-2.5 text-sm font-semibold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">{{ __('New Sale') }}</button>
            </div>
        </div>
    </Modal>
</template>
