...
watch(flash, (val) => {
    ...
    if (val?.type === 'import_queued') {
        addOrUpdate({
            ...
            export_key: val.import_type ?? 'import',
            kind: 'import',
            status: 'queued',
        });
        hasBeenTriggered.value = true;
...
function humanize(item) {
    const key = item.kind === 'import' ? item.import_type : item.export_key;
    if (!key) return item.kind === 'import' ? __('Import') : __('Export');
    const label = __(key.replace(/[-_]/g, ' ').replace(/\b\w/g, l => l.toUpperCase()));
    return item.kind === 'import' ? `${label} — ${__('Import')}` : label;
}
                    ...
                    <template v-if="activeExports.length">
                        ...
                        <div v-for="(exp, i) in activeExports" :key="exp.id"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-[13px] font-semibold text-gray-900 dark:text-white truncate">{{ humanize(exp) }}</div>
                        ...
                        <div v-for="(exp, i) in doneExports" :key="exp.id"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-1.5 mt-0.5">
                                    ...
                                    <template v-else>
                                        <span class="text-[11px] text-red-500 dark:text-red-400 truncate">{{ exp.failure_message || (exp.kind === 'import' ? __('Import failed.') : __('Export failed.')) }}</span>
                                    </template>
                                </div>
                            </div>
                        </div>
        ...
        >
            ...
            >
                ...
                <span class="text-sm font-semibold text-gray-700 dark:text-gray-300 whitespace-nowrap">
                    ...
                    <template v-else-if="pillState === 'active'">{{ activeExports.length }} {{ __('in progress') }}</template>
