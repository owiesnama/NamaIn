...
Use this skill when the user asks to run the migrated source command `finalize`.
...
4. run the full available suite, including `php artisan test --parallel`.
...
You must not claim the work is fully verified unless the relevant full testsuite actually ran successfully.
