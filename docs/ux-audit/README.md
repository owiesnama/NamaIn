...
| Item | Title | Why held | Blocked on | Size |
|---|---|---|---|---|
| [U1](U1-configurable-numerals.md) | Configurable numerals (Arabic-Indic ↔ Latin) as a tenant preference | Design invalidated by testing — `<Ltr>` can't stay a universal wrapper | Design decision (§4) | L |
| [U2](U2-arabic-translation-coverage.md) | 175 untranslated `__()` keys + CI coverage gate | Needs a native Arabic pass | Owner (translation) | M — mostly not code |
| [U3](U3-rtl-root-migration.md) | Remove the nested `dir` divs now `<html dir>` ships | Wants PR #77 confirmed in production first | PR #77 deployed | S |
| [U4](U4-mobile-density.md) | Mobile products table → cards; remaining 27 sub-44px targets; export pill collision | Real design work, not a class swap | — | M |
| [U5](U5-physical-direction-classes.md) | Physical `text-right pr-8` in quote/invoice totals | **Never visually confirmed** — flagged from source only | Verification (§3) | S |
